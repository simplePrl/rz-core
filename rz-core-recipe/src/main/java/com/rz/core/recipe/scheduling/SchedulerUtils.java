...
public class SchedulerUtils {
    // protected static Logger log = Logger.getLogger(SchedulerUtils.class);
    ...
    public static StdSchedulerFactory initStdSchedulerFactory() {
        ...
        try {
            ...
            log.error("初始化StdSchedulerFactory失败");
    ...
    public static JobDetail assemblyJobDetail(JobDetailProxyBean jobDetail) {
        ...
        if (StringUtils.isNotBlank(key)) {
            ...
            jobBuilder = jobBuilder.withIdentity(Helper.generatorUUID("JOB"), jobDetail.getGroup());
        }
     ...
     */
    public static CronTrigger assemblyCronTrigger(CronTriggerBean cronTriggerBean) throws ParseException {
        ...
        if (StringUtils.isNotBlank(key)) {
            ...
            triggerBuilder = triggerBuilder.withIdentity(Helper.generatorUUID("CronTrigger"), cronTriggerBean.getGroup());
        }
        // 设置描述
        ...
        if (StringUtils.isNotBlank(cronTriggerBean.getStartTime())) {
            triggerBuilder = triggerBuilder.startAt(DateUtils.parseDate(cronTriggerBean.getStartTime(), "yyyy-MM-dd HH:mm:ss"));
        } else {
            triggerBuilder = triggerBuilder.startNow(); // 当启动时间为空默认立即启动调度器
        }
        // 设置结束时间
        if (StringUtils.isNotBlank(cronTriggerBean.getEndTime())) {
            triggerBuilder = triggerBuilder.endAt(DateUtils.parseDate(cronTriggerBean.getEndTime(), "yyyy-MM-dd HH:mm:ss"));
        }
        // 设置优先级
     ...
     * 装配简单触发器
     * 
     * @param simpleTriggerBean
     *            简单触发器
     * @return 简单触发器
     * @throws ParseException
     */
    public static SimpleTrigger assemblySimpleTrigger(SimpleTriggerBean simpleTriggerBean) throws ParseException {
        TriggerBuilder triggerBuilder = TriggerBuilder.newTrigger();
        ...
        if (StringUtils.isNotBlank(key)) {
            ...
            triggerBuilder = triggerBuilder.withIdentity(Helper.generatorUUID("SimpleTrigger"), simpleTriggerBean.getGroup());
        }
        // 设置描述
        ...
        if (StringUtils.isNotBlank(simpleTriggerBean.getStartTime())) {
            triggerBuilder = triggerBuilder.startAt(DateUtils.parseDate(simpleTriggerBean.getStartTime(), "yyyy-MM-dd HH:mm:ss"));
        } else {
            triggerBuilder = triggerBuilder.startNow(); // 当启动时间为空默认立即启动调度器
        }
        // 设置结束时间
        if (StringUtils.isNotBlank(simpleTriggerBean.getEndTime())) {
            triggerBuilder = triggerBuilder.endAt(DateUtils.parseDate(simpleTriggerBean.getEndTime(), "yyyy-MM-dd HH:mm:ss"));
        }
        // 设置优先级
